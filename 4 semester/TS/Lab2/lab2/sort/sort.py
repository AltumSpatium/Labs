import os, heapq, argparse, sys

class BigDataSort(object):
	def __init__(self, f_input='in.txt', f_output='out.txt', line_separator='\n', field_separator='\t',
				 key=None, numeric=False, need_check=False, reverse=False, buffersize=1000000):
	...
	def check(self, f_input):
		with open(f_input) as f_inp:
			...
			while line:
				...
				if nextline and ((line > nextline and not self.reverse) or (line < nextline and self.reverse)):
					return False
				line = nextline
		return True
...
def main():
	...
	bds = BigDataSort(f_input=arguments.input, f_output=arguments.output, line_separator=arguments.lineseparator,
					  field_separator=arguments.fieldseparator, key=arguments.key, numeric=arguments.numeric,
					  need_check=arguments.check, reverse=arguments.reverse, buffersize=arguments.buffersize)
