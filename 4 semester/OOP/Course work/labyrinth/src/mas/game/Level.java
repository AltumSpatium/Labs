...
public class Level
{
    ...
    public Vector<Ghost> ghosts = new Vector<Ghost>();

    public Primitive[][] levelmap = null;
    ...
    private boolean init (Hero _hero)
    {
        ...
        if (_hero.levelmap == null)
        {       
            if ((levelmap = Builder.loadLevel("@" + _hero.level + ".lev", config, null)) == null)
                return false;
        }
        else if ((levelmap = Builder.loadLevel("@" + _hero.level + ".lev", config, _hero.levelmap)) == null)
            return false;
        ...
        if (init(_hero))
        {
            ...
            while (true)
            {
                if (updatestep <= 0)
                {
                    ...
                    if (keyexit)
                    {
                        hero.levelmap = Builder.saveLevel(this);
                        _hero.copyState(hero);                      
