...
public class Utils
{
  ...
  public static int toHex (byte[] _srcdata, int _srcpos, int _srcsize, byte[] _dstdata, int _dstpos, boolean _getsizeonly)
  {
      final byte hexmap[] = "0123456789ABCDEF".getBytes();
      
      if(!_getsizeonly)
      {
          _srcsize = _srcpos + _srcsize;
        
          while(_srcpos < _srcsize)
          {
              _dstdata[_dstpos++] = hexmap[(_srcdata[_srcpos] >> 4) & 0x0F];
              _dstdata[_dstpos++] = hexmap[_srcdata[_srcpos++] & 0x0F];
          }
      }
      return (_srcsize << 1);
  }
  ...
  public static int fromHex (byte[] _srcdata, int _srcpos, int _srcsize, byte[] _dstdata, int _dstpos, boolean _getsizeonly)
  { 
      byte symbol;
      int size = _dstpos;
      ...
      if(!_getsizeonly)
      {
          ...
          while(_srcpos < _srcsize)
          {
              symbol = _srcdata[_srcpos++];
              
              if(symbol >= '0' && symbol <= '9') symbol -= '0';
              else if(symbol >= 'A' && symbol <= 'F') symbol -= ('A' - 10);
              else if(symbol >= 'a' && symbol <= 'f') symbol -= ('a' - 10);
              else continue;
              
              value = (byte) ((symbol & 0x0F) << 4); 
              ...
              while(_srcpos < _srcsize)
              {
                  ...
                  value |= (byte) (symbol & 0x0F);
                  _dstdata[_dstpos++] = value;
