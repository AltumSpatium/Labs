...
public class Hero extends Person
{
    ...
    public void saveProfile ()
    {
        ...
        if (levelmap != null && levelmap.length() > 0)
        {
            byte[] srcdata = levelmap.getBytes();
            byte[] dstdata = new byte[srcdata.length << 1]; 
              
            Utils.toHex(srcdata, 0, srcdata.length, dstdata, 0, false);
            profile.append("\r\nhero.levelmap=" + new String(dstdata));       
        }
    ...
    public void loadProfile ()
    {
        ...
        if (parameters != null)
        {
            ...
            for (String line : parameters)
            {
                ...
                else if (values[0].trim().equalsIgnoreCase("hero.levelmap"))
                {
                    byte[] srcdata = values[1].trim().getBytes();
                    byte[] dstdata = new byte[srcdata.length >> 1]; 
                    
                    Utils.fromHex(srcdata, 0, srcdata.length, dstdata, 0, false);
                    levelmap = new String(dstdata);
                }
            }
        }
    }
