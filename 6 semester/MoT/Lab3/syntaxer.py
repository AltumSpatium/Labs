...
def p_arithmetic_expression(p):
    ...
    if len(p) == 4:
        if p[2] == '/' and p[3] == '0':
            lines = p.lexer.lexdata.split('\n')
            p_line, p_col = get_line_col(lines, p[3])
            error_str = "Error! Divide by zero: line: {0}, col: {1}".format(p_line, p_col)
            Node.errors.append(error_str)
    if len(p) == 2:
        p[0] = Node('expression', [p[1]])
    elif len(p) == 5:
...
def build_tree(code):
    global id_table
    id_table, errors = create_id_table()
    if errors:
        Node.errors.extend(errors)
    find_dbz(code)
    return (parser.parse(code), id_table)
